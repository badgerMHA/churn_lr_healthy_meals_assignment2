...
def load_artifacts():
    ...
    model_path = base_dir / "churn_lr_healthy_meals (1).pkl"
    encoder_path = base_dir / "churn_encoder_healthy_meals (1).pkl"
...
with col2:
    ...
    income_level = st.selectbox(
        "Income Level",
        encoder["encoder"].categories_[0].tolist()
    )

    education = st.selectbox(
        "Education",
        encoder["encoder"].categories_[1].tolist()
    )

    device_type = st.selectbox(
        "Device Type",
        encoder["encoder"].categories_[2].tolist()
    )
